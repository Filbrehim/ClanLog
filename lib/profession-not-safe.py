    ...
    * retenir les fichiers déja visités
    ...
    def __init__(self,qui,/,arguments = None) :
        ...
        * utilise les fichiers `messages.csv` et `compliments.csv` pour construire l'analyse.
        ...
        self.moi           = qui
        self.appréciation_re = re.compile('^(?P<qui>[A-Za-z ]+) says, "Hail, ' + qui + '.\s+(?P<comment>[A-Za-z ]+)."')
...
# en attente génération csv digne de ce nom
        ...
        * le dernier professeur (sauf pour les chemins)
        ...
        if tmp_app is not None :
            ...
            if p in self.appréciations :
                print(self.appréciations[p])
                tmp_comment_avant = self.appréciations[p]['comment']
                self.appréciations[p]['comment'] = comment
                if tmp_comment_avant == comment :
                    self.appréciations[p]['max_caché'] = self.appréciations[p]['max_caché'] - self.appréciations[p]['niveau']
            else :
                self.appréciations[p] = {'niveau' : 0 ,
                                         'comment' : comment}
                if 'max' in  self.compliments[comment] :
                    self.appréciations[p]['max_caché'] = self.compliments[comment]['max']
            return 1
    ...
    def correction_globale(self) :
        ...
        * cf http://gorvin.50webs.com/CLRev/healer.html
        """
        self.correction_multiple('Faustus',[('Eva',0.333),('Sprite',0.30)])
        self.correction_multiple('Higgrus',[('Eva',0.25) ,('Sprite',0.10),('Master Bodrus',0.2)])
        self.correction_multiple('Sespus',[('Eva',0.275),('Sprite',0.24)])
        self.correction_multiple('Respia',[('Eva',0.375),('Sprite',0.20)])
